...
request_status_msg_t handle_received_data_at_container(distribution_container *container, char *data, struct sockaddr_in remote, int connected_fd){
		...
		for(i=0; i<FD_SETSIZE; i++){
			if(FD_ISSET(i, &read_fd_set)){
				if(i==sock){
					...
					//read data/parse
					bytesRead = recvfrom(i, msg_buffer, MAX_MSG_SIZE, 0, (struct sockaddr *)&remote, sizeof(remote));
					if(bytesRead<0){
						//error handling
					}else if(bytesRead==0){
...
int handle_new_connection_request(uint32_t room_id, uint32_t user_id, container_connection_info_t *container_connection_info){
	int ret = 1;
	void *found;
	if(NULL == (found = hashtable_search(&distribution_containers_for_room_id, &room_id))){
		...
		//start threads
		if((ret = pthread_create(&new_state->worker_threads[0], &attr, container_connection_listener, (void *)&new_state->container)) != 0){
			//error handling
		}

		if((ret = pthread_create(&new_state->worker_threads[1], &attr, container_distribute_recv_data, (void *)&new_state->container)) != 0){
			//error handling
		}
